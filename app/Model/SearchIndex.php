        ...
        if ($primary) {
            foreach($results as $x => $result) {
                if (!empty($result['SearchIndex'])) {
					...
					if(isset($result['SearchIndex']['model']))
					{
						$Model = ClassRegistry::init($result['SearchIndex']['model']);
					}
					else
					{
						$DbResult = $this->SearchIndex->find('first', array( 'conditions'=>array('id'=>$result['SearchIndex']['id']) ) );
						$Model = ClassRegistry::init($DbResult['SearchIndex']['model']);
					}
